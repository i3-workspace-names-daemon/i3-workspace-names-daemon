...
class TestValidateConfig(unittest.TestCase):
    ...
    def test_invalid_icon_name(self):
        config = {"app1": "does-not-exist"}
        err = _validate_config(config)
        self.assertTrue(err)

    def test_custom_pango_markup(self):
        config = {"emacs": u'<span font_desc="file-icons">\ue926</span>'}
        err = _validate_config(config)
        self.assertFalse(err)
    ...
    def test_icon_mapping_explicit_invalid(self):
        config = {"app2": {"icon": "does-not-exist"}}
        err = _validate_config(config)
        self.assertTrue(err)
    ...
    def test_mapping_transform_title(self):
        config = {"appbla": {"transform_title": {"from": ":(.*)", "to": r"pj: \1",}}}
        err = _validate_config(config)
        self.assertFalse(err)

    def test_mapping_transform_title_missing_from(self):
        config = {"appbla": {"transform_title": {"to": r"pj: \1",}}}
        err = _validate_config(config)
        self.assertTrue(err)

    def test_mapping_transform_title_missing_to(self):
        config = {"appbla": {"transform_title": {"from": ":(.*)",}}}
        err = _validate_config(config)
        self.assertTrue(err)
